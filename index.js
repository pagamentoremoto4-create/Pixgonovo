...
.theme-photo-banner{position:relative;height:250px;overflow:hidden;border-radius:24px;margin-bottom:18px;border:1px solid color-mix(in srgb,var(--accent) 45%,transparent);box-shadow:0 18px 45px rgba(0,0,0,.38),0 0 28px color-mix(in srgb,var(--accent) 16%,transparent);background:#020617}.theme-photo-banner img{width:100%;height:100%;display:block;object-fit:cover;object-position:center 20%}.theme-photo-banner:after{content:'';position:absolute;inset:0;background:linear-gradient(90deg,rgba(0,0,0,.72),rgba(0,0,0,.12) 58%,rgba(0,0,0,.28))}.theme-photo-copy{position:absolute;left:24px;bottom:22px;z-index:2;max-width:62%}.theme-photo-copy h1{margin:0 0 5px;font-size:28px}.theme-photo-copy b{color:var(--accent)}.theme-photo-copy p{margin:0;color:#d8e3ef}.theme-preview img{width:100%;height:100%;object-fit:cover;display:block;border-radius:inherit}.theme-card .theme-preview{padding:0;overflow:hidden}.theme-card .theme-preview:after{content:'';position:absolute}.theme-auto-note{padding:10px 12px;border-radius:13px;background:color-mix(in srgb,var(--accent) 10%,#08111f);border:1px solid color-mix(in srgb,var(--accent) 28%,transparent);margin:10px 0;color:#dbe7f5}.theme-auto-note b{color:var(--accent)}@media(max-width:900px){.theme-photo-banner{height:190px;border-radius:18px}.theme-photo-copy{left:16px;bottom:15px;max-width:86%}.theme-photo-copy h1{font-size:22px}}@media(max-width:900px){.admin-head{margin:-14px -14px 16px;padding:10px 14px}.head-brand span,#systemText{display:none}.head-status{padding:7px 9px}.layout,.layout.side-collapsed{grid-template-columns:1fr}.side{position:fixed;left:-300px;top:0;width:280px;height:100vh;z-index:80;transition:left .22s ease}.layout.mobile-open .side{left:0}.layout.side-collapsed .side a span,.layout.side-collapsed .nav-title,.layout.side-collapsed .brand-text,.layout.side-collapsed .side-profile{display:initial}.layout.side-collapsed .side a{justify-content:flex-start;font-size:13px}.main{padding:14px}.notif-bell{top:auto;bottom:18px}.theme-grid{grid-template-columns:1fr}}
...
async function enviarWhatsAppTexto(numero, text) {
  ...
  try {
    if (WHATSAPP_PROVIDER === 'baileys' || WHATSAPP_PROVIDER === 'qrcode') {
      ...
      if (!sock) {
        console.log('⚠️ Nenhum WhatsApp com função Bot de Serviços está conectado.');
        return false;
      }
...
async function numeroPodeAdministrarPorWhatsApp(numero) {
  ...
  const autorizado = await get(`SELECT id FROM destinatarios_avisos
...
async function listarImeisServicoAdminWhatsApp(from, servico) {
  ...
  const linhas = rows.map(r => `${r.imei || r.entrada_valor || '-'} — ${r.status}`).join('\n');
  await enviarTexto(from, `📋 *${servico.servico_nome}*\n\n${linhas}`);
}
...
async function tratarComandosAdminWhatsApp(numeroNorm, from, textoOriginal) {
  ...
  if (sess?.etapa === 'wa_admin_pedidos_servico') {
    ...
    if (!servico) {
      await enviarTexto(from, '❌ Serviço inválido. Digite *pedidos* para começar novamente.');
      adminSessao.delete(from);
      return true;
    }
  ...
  if (sess?.etapa === 'wa_admin_buscar_imei') {
    ...
    if (!/^\d{15}$/.test(imei)) {
      await enviarTexto(from, '❌ IMEI inválido. Digite os 15 números do IMEI.');
      return true;
    }
    const pedido = await get(`SELECT * FROM pedidos
    ...
    adminSessao.set(from, { etapa: 'wa_admin_buscar_acao', pedidoId: pedido.id });
    await enviarTexto(from, `🔎 *PEDIDO LOCALIZADO*\n\n🛠 Serviço: ${pedido.servico_nome || '-'}\n📱 IMEI: ${pedido.imei || pedido.entrada_valor || '-'}\n📍 Status: ${pedido.status}\n\n1️⃣ EM PROCESSO\n2️⃣ FINALIZAR\n\nDigite 1 ou 2.`);
    return true;
  }

  if (sess?.etapa === 'wa_admin_buscar_acao') {
    if (texto !== '1' && texto !== '2') {
      await enviarTexto(from, '❌ Opção inválida. Digite *1* para EM PROCESSO ou *2* para FINALIZAR.');
      return true;
    }
    const pedido = await get('SELECT * FROM pedidos WHERE id=?', [sess.pedidoId]);
    adminSessao.delete(from);
    if (!pedido) {
    ...
    if (texto === '1') {
      ...
      await enviarTexto(from, `✅ Pedido atualizado para *EM PROCESSO*.\n\n📱 ${atualizado.imei || atualizado.entrada_valor || '-'}\n🛠 ${atualizado.servico_nome || '-'}`);
      return true;
    }
    if (pedido.status === 'FINALIZADO') {
      await enviarTexto(from, `ℹ️ Este pedido já está FINALIZADO.\n\n📱 ${pedido.imei || pedido.entrada_valor || '-'}`);
      return true;
    }
    await finalizarPedido(pedido);
    await enviarTexto(from, `✅ Pedido *FINALIZADO*.\n\n📱 ${pedido.imei || pedido.entrada_valor || '-'}\n🛠 ${pedido.servico_nome || '-'}`);
    return true;
  }
...
async function processarMensagemWhatsApp({ numero, nome, texto }) {
  ...
  if (await tratarComandosAdminWhatsApp(numeroNorm, from, textoOriginal)) return;
...
async function garantirMigracaoSessoesLegadas() {
  ...
  if (sessaoWhatsAppRegistrada(WHATSAPP_SESSION_DIR)) {
    ...
    await run(`INSERT INTO whatsapp_sessoes (nome,session_key,session_dir,funcao_bot,funcao_grupos,funcao_status,ativo)
               VALUES (?,?,?,?,?,?,1)`, ['Bot de Serviços', 'legacy-services', WHATSAPP_SESSION_DIR, funcoes.bot ? 1 : 0, funcoes.anuncios ? 1 : 0, 0]);
    console.log('✅ V87: sessão antiga do Bot de Serviços importada.');
  }
  if (sessaoWhatsAppRegistrada(WHATSAPP_ADS_SESSION_DIR)) {
    await run(`INSERT INTO whatsapp_sessoes (nome,session_key,session_dir,funcao_bot,funcao_grupos,funcao_status,ativo)
               VALUES (?,?,?,?,?,?,1)`, ['Anúncios em Grupos', 'legacy-ads', WHATSAPP_ADS_SESSION_DIR, 0, 1, 0]);
...
async function carregarSessoesWhatsApp() {
  ...
  for (const [id, sessao] of whatsappSessoes) {
    if (!ids.has(id) && !sessao.conectado && !sessao.iniciando) whatsappSessoes.delete(id);
  }
...
function agendarReconexaoSessaoMulti(sessao) {
  ...
  sessao.timer = setTimeout(() => {
    ...
    if (!sessaoWhatsAppRegistrada(sessao.sessionDir)) {
      ...
      sessao.erro = 'Credenciais não registradas. Gere um novo QR Code.';
      emitirStatusSessaoMulti(sessao);
      return;
    }
    console.log(`🔄 V87: reconectando automaticamente ${sessao.nome}.`);
    iniciarSessaoWhatsAppMulti(sessao.id, { modo: 'restaurar' }).catch(e => console.log(`❌ V87 RECONEXÃO #${sessao.id}:`, e.message));
  }, 5000);
}

async function iniciarSessaoWhatsAppMulti(id, opcoes = {}) {
  if (!WHATSAPP_ENABLED || !['baileys','qrcode'].includes(WHATSAPP_PROVIDER)) return;
      ...
      try {
        ...
        if (connection === 'open') {
          ...
          if (sessao.funcaoBot && (!whatsappSocket || !conectado)) {
            whatsappSocket = socketAtual; conectado = true; whatsappNumeroConectado = sessao.numero; whatsappStatus = 'CONECTADO';
          }
          emitirStatusSessaoMulti(sessao);
          notificarPainel('whatsapp', `✅ ${sessao.nome} conectado`, `${sessao.numero || 'Sessão ativa'} — ${descricaoFuncoesSessao(sessao)}`);
        }
        if (connection === 'close') {
          if (sessao.socket !== socketAtual) return;
...
async function obterSessaoBotConectada() {
  ...
  return Array.from(whatsappSessoes.values()).find(s => s.ativo && s.funcaoBot && s.conectado && s.socket) || null;
}
...
async function obterSessoesStatusConectadas() {
  if (!whatsappSessoesCarregadas) { try { await carregarSessoesWhatsApp(); } catch (_) {} }
  return Array.from(whatsappSessoes.values()).filter(s => s.ativo && s.funcaoStatus && s.conectado && s.socket);
}
...
async function destinatariosStatusConhecidos() {
  const rows = await all(`SELECT whatsapp,jid FROM revendas WHERE status != 'REMOVIDA'`);
  const jids = new Set();
  for (const r of rows) {
...
async function publicarStatusWhatsApp({ texto='', imagemBuffer=null }) {
  ...
  if (!sessoes.length) throw new Error('Nenhum WhatsApp com a função Anúncios no Status está conectado.');
  const statusJidList = await destinatariosStatusConhecidos();
  if (!statusJidList.length) throw new Error('Não há clientes com WhatsApp cadastrado para compor a audiência do Status.');
  let enviados=0, falhas=0;
  for (const sessao of sessoes) {
    try {
      const conteudo = imagemBuffer ? { image: imagemBuffer, caption: String(texto || '') } : { text: String(texto || '') };
      await sessao.socket.sendMessage('status@broadcast', conteudo, { broadcast: true, statusJidList });
      enviados++;
    } catch (e) { falhas++; console.log(`❌ STATUS WHATSAPP #${sessao.id}:`, e.message); }
  }
...
async function sessaoParaAnuncios() {
  ...
  if (multi) return { socket: multi.socket, conectado: true, numero: multi.numero, origem: `multi:${multi.id}`, label: multi.nome };
  ...
  if (ads?.socket && ads?.conectado) return { socket: ads.socket, conectado: true, numero: ads.numero, origem: 'ads', label: 'Anúncios' };
  return null;
}
...
app.get('/admin', async (req, res) => {
  ...
  table += '</table>';
  res.send(page('Dashboard', `<div data-live-dashboard="1"></div><div class="theme-photo-banner"><img src="${TEMAS_PAINEL[temaAtual()].imagem}?v=91" alt="Banner ${safeHtml(TEMAS_PAINEL[temaAtual()].nome)}"><div class="theme-photo-copy"><h1>CentralUnlocker <b>${safeHtml(TEMAS_PAINEL[temaAtual()].nome)}</b></h1><p>Banner automático do tema selecionado.</p></div></div><div class="hero-hacker"><div class="hero-content"><div class="eyebrow">Painel seguro</div><h1>Painel <span>CentralUnlocker</span></h1><p>Controle total de pedidos, revendas, saldo, IMEI, Lock Code e serviços manuais.</p></div><div class="system-card"><h3>Status do sistema</h3><div class="system-row"><span>API Principal</span><span class="online">ONLINE</span></div><div class="system-row"><span>Bot Telegram</span><span class="online">${tgBot ? 'CONECTADO' : 'OFFLINE'}</span></div><div class="system-row"><span>WhatsApp</span><span class="online">${conectado ? 'CONECTADO' : whatsappStatus}</span></div><div class="system-row"><span>Processador</span><span class="online">ONLINE</span></div><div class="system-row"><span>Banco de Dados</span><span class="online">ONLINE</span></div></div></div><div class="topbar"><h1>Resumo geral</h1><span class="clock-box">🕒 ${dateBR(new Date())}</span></div><div class="grid">
...
app.get('/admin/whatsapp/status', async (req, res) => {
  ...
  const msg = req.query.ok ? `<div class="card"><p style="color:#22c55e">✅ ${safeHtml(req.query.ok)}</p></div>` : req.query.erro ? `<div class="card"><p style="color:#ef4444">❌ ${safeHtml(req.query.erro)}</p></div>` : '';
  res.send(page('Status WhatsApp', `<h1>🟢 Anúncios no Status</h1>${msg}<div class="card"><p><b>Sessões configuradas:</b> ${sessoes.length} &nbsp; <b>Conectadas:</b> ${conectadas}</p><p class="mini-help">O Status será publicado em todos os números conectados que estiverem com “Anúncios no Status” marcado. A audiência usa os clientes com WhatsApp cadastrados no sistema.</p></div><div class="card"><h2>➕ Publicar agora</h2><form method="post" action="/admin/whatsapp/status/publicar"><label>Foto opcional</label><input id="statusFile" type="file" accept="image/png,image/jpeg,image/webp"><input id="statusImage" type="hidden" name="imageData"><img id="statusPreview" style="display:none;max-width:100%;max-height:300px;margin-top:10px;border-radius:12px"><label>Texto / legenda</label><textarea name="texto" rows="7" maxlength="4000" placeholder="Digite o anúncio..."></textarea><br><button class="btn green">🟢 Publicar no Status</button> <a class="btn" href="/admin/whatsapp">Voltar</a></form><script>const f=document.getElementById('statusFile'),d=document.getElementById('statusImage'),p=document.getElementById('statusPreview');f&&f.addEventListener('change',()=>{const file=f.files&&f.files[0];if(!file){d.value='';p.style.display='none';return}const r=new FileReader();r.onload=()=>{d.value=r.result;p.src=r.result;p.style.display='block'};r.readAsDataURL(file)});</script></div>`));
});
...
app.post('/admin/whatsapp/status/publicar', async (req, res) => {
  try {
    ...
    if (!texto && !imagemBuffer) throw new Error('Digite um texto ou escolha uma imagem.');
    const r = await publicarStatusWhatsApp({ texto, imagemBuffer });
    res.redirect('/admin/whatsapp/status?ok=' + encodeURIComponent(`Publicado em ${r.enviados} sessão(ões). Audiência: ${r.audiencia}. Falhas: ${r.falhas}.`));
  } catch (e) { res.redirect('/admin/whatsapp/status?erro=' + encodeURIComponent(e.message)); }
});
...
app.post('/admin/whatsapp/:id/salvar', async (req, res) => {
  ...
  await run('UPDATE whatsapp_sessoes SET nome=?,funcao_bot=?,funcao_grupos=?,funcao_status=?,atualizado_em=CURRENT_TIMESTAMP WHERE id=?', [nome,bot,grupos,status,id]);
  const row = await get('SELECT * FROM whatsapp_sessoes WHERE id=?', [id]);
...
app.post('/admin/whatsapp/:id/desconectar', async (req, res) => {
  ...
  await run('UPDATE whatsapp_sessoes SET numero=NULL, atualizado_em=CURRENT_TIMESTAMP WHERE id=?', [id]);
  const s = whatsappSessoes.get(id); if (s) s.numero = '';
...
app.post('/admin/whatsapp/:id/excluir', async (req, res) => {
  ...
  if (row) {
    await desconectarSessaoWhatsAppMulti(id, true);
    try { fs.rmSync(row.session_dir, { recursive: true, force: true }); } catch (_) {}
