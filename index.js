...
const BOTZAP_SECRET = process.env.BOTZAP_SECRET || '';
const BOTZAP_ENABLED = String(process.env.BOTZAP_ENABLED || '1') === '1';
...
const TEMAS_PAINEL = {
  ...
  'gold-vip': { nome: '🟡 Gold VIP', cor: '#ffd166', cor2: '#ff9f1c' },
  'matrix-hacker': { nome: '💻 Matrix Hacker', cor: '#00ff41', cor2: '#39ff14' },
  'cyber-security': { nome: '🌐 Cyber Security', cor: '#00d4ff', cor2: '#00ffcc' },
  'black-elite': { nome: '⚫ Black Elite', cor: '#00ff66', cor2: '#c0c0c0' }
};

const pedidoSessao = new Map();
const adminSessao = new Map();
const botzapRespostas = new Map();
...
function botzapJid(numero) {
  const n = normalizarNumeroWhatsApp(numero);
  return n ? `${n}@botzap` : '';
}
function isBotzapJid(jid) {
  return String(jid || '').endsWith('@botzap');
}
function textoBotzapBody(body) {
  return String(
    body?.mensagem ||
    body?.message ||
    body?.text ||
...
function telefoneBotzapBody(body) {
...
function nomeBotzapBody(body) {
  return String(body?.nome || body?.name || body?.contact_name || body?.pushName || 'Cliente').trim() || 'Cliente';
}
function criarMensagemBotzap({ telefone, mensagem, nome }) {
  const jid = botzapJid(telefone);
  return {
    _botzap: true,
    key: {
      remoteJid: jid,
      id: `botzap_${Date.now()}_${Math.random().toString(16).slice(2)}`,
      fromMe: false
    },
...
function respostaPadraoBotzap(textos) {
  const limpos = (textos || []).filter(Boolean).map(String);
  return limpos.join('\\n\\n') || '✅ Recebido.';
}
...
function isPhoneJid(jid) { return String(jid || '').endsWith('@s.whatsapp.net') || isBotzapJid(jid); }
...
function imagemTemaArquivo(theme) {
  const map = {
    'hacker-green': 'hacker_green.svg',
    'hacker-blue': 'hacker_blue.svg',
    'hacker-red': 'hacker_red.svg',
    'hacker-purple': 'hacker_purple.svg',
    'dark-pro': 'hacker_blue.svg',
    'gold-vip': 'hacker_gold.svg',
    'matrix-hacker': 'hacker_matrix.svg',
    'cyber-security': 'hacker_cyber.svg',
    'black-elite': 'hacker_elite.svg'
  };
  return map[theme] || map['hacker-green'];
}
function imagemTemaUrl(theme) {
  return `/themes/${imagemTemaArquivo(theme)}`;
}
function ensureHackerModelos() {
  const modelos = [
    ...
    ['hacker_matrix.svg', '#00ff41', '#39ff14', 'MATRIX HACKER'],
    ['hacker_cyber.svg', '#00d4ff', '#00ffcc', 'CYBER SECURITY'],
    ['hacker_elite.svg', '#00ff66', '#c0c0c0', 'BLACK ELITE']
  ];
...
<defs>
<linearGradient id="g" x1="0" x2="1" y1="0" y2="1"><stop offset="0" stop-color="#020617"/><stop offset="1" stop-color="#111827"/></linearGradient>
<radialGradient id="r" cx="70%" cy="35%" r="60%"><stop offset="0" stop-color="${c1}" stop-opacity=".38"/><stop offset="1" stop-color="${c2}" stop-opacity="0"/></radialGradient>
</defs>
<rect width="1200" height="650" fill="url(#g)"/><rect width="1200" height="650" fill="url(#r)"/>
<g opacity=".18" font-family="monospace" font-size="22" fill="${c1}">
${Array.from({length:18}).map((_,i)=>`<text x="${20+(i%3)*390}" y="${40+i*34}">010101 ACCESS ${title} 101010</text>`).join('')}
</g>
<g transform="translate(705 76)">
<circle cx="165" cy="125" r="96" fill="#0b1220" stroke="${c1}" stroke-width="5"/>
<path d="M48 322c38-118 184-154 270-47 32 40 47 95 51 162H0c8-47 22-87 48-115z" fill="#0b1220" stroke="${c1}" stroke-width="6"/>
<path d="M89 116c58 42 115 45 180 0 0 74-28 121-91 139-63-17-89-65-89-139z" fill="#111827" stroke="${c2}" stroke-width="5"/>
<rect x="94" y="143" width="168" height="45" rx="22" fill="#020617" stroke="${c1}" stroke-width="4"/>
<circle cx="142" cy="165" r="7" fill="${c1}"/><circle cx="214" cy="165" r="7" fill="${c1}"/>
</g>
<text x="70" y="505" font-family="Arial Black,Arial" font-size="58" fill="${c1}">${title}</text>
<text x="72" y="555" font-family="Arial" font-size="28" fill="#eaf0f8">Painel profissional • eSIM • Revendas • PIX</text>
</svg>`;
      fs.writeFileSync(fp, svg);
    }
  }
}
function aplicarImagemTema(theme) {
  ensureHackerModelos();
  const src = path.join(THEME_DIR, imagemTemaArquivo(theme));
  if (!fs.existsSync(PUBLIC_IMG_DIR)) fs.mkdirSync(PUBLIC_IMG_DIR, { recursive: true });
  if (fs.existsSync(src)) fs.copyFileSync(src, HACKER_IMAGE_PATH);
}
...
async function enviarTexto(to, text) {
  if (!to) return;
  if (isBotzapJid(to)) {
    const arr = botzapRespostas.get(to) || [];
    arr.push(String(text || ''));
    botzapRespostas.set(to, arr);
    return;
  }
...
async function enviarImagem(to, filePath, caption='') {
  ...
  if (isBotzapJid(to)) {
    const arr = botzapRespostas.get(to) || [];
    arr.push(`${caption || '📎 Arquivo enviado'}\n\n${BASE_URL ? BASE_URL : ''}/esim/${path.basename(filePath)}`);
    botzapRespostas.set(to, arr);
    return true;
  }
...
app.post('/webhook/botzap', async (req, res) => {
  try {
    if (!BOTZAP_ENABLED) return res.status(403).json({ ok: false, resposta: 'BotZap desativado.' });

    if (BOTZAP_SECRET) {
      const auth = req.headers.authorization || req.headers['x-botzap-secret'] || '';
      const token = String(auth).replace(/^Bearer\s+/i, '');
      if (token !== BOTZAP_SECRET) {
        return res.status(401).json({ ok: false, resposta: 'Não autorizado.' });
      }
    }

    const telefone = telefoneBotzapBody(req.body);
    const mensagem = textoBotzapBody(req.body);
    const nome = nomeBotzapBody(req.body);

    console.log('========== BOTZAP ==========');
    console.log({ telefone, nome, mensagem, body: req.body });

    if (!telefone) {
      return res.json({ ok: false, resposta: 'Telefone não recebido pelo BotZap.' });
    }

    if (!mensagem) {
      return res.json({ ok: true, resposta: 'Envie uma mensagem de texto. Digite menu para começar.' });
    }

    const from = botzapJid(telefone);
    botzapRespostas.set(from, []);

    const msg = criarMensagemBotzap({ telefone, mensagem, nome });
    await tratarWhatsApp(msg, from, mensagem, mensagem.toLowerCase(), isAdminJid(from), nome);

    const respostas = botzapRespostas.get(from) || [];
    botzapRespostas.delete(from);

    const resposta = respostaPadraoBotzap(respostas);
    ...
    console.log('❌ ERRO WEBHOOK BOTZAP:', e);
    return res.status(500).json({
      ok: false,
      success: false,
      resposta: '❌ Erro interno na CentralUnlocker.',
      error: e.message
    });
  }
});

app.get('/webhook/botzap', (req, res) => {
  res.json({ ok: true, resposta: 'Webhook BotZap CentralUnlocker online ✅' });
});
...
app.post('/admin/config/theme', async (req, res) => { const theme = String(req.body.theme || 'hacker-green'); if (TEMAS_PAINEL[theme]) { PAINEL_TEMA = theme; await setConfig('painel_tema', theme); aplicarImagemTema(theme); notificarPainel('tema', '🎨 Tema e foto alterados', TEMAS_PAINEL[theme].nome); } res.redirect('/admin/config'); });
...
if (!fs.existsSync(THEME_DIR)) fs.mkdirSync(THEME_DIR, { recursive: true });
    ...
    fs.writeFileSync(HACKER_IMAGE_PATH, buff);
    fs.writeFileSync(path.join(THEME_DIR, `custom_hacker.${m[1].toLowerCase()==='jpeg'?'jpg':m[1].toLowerCase()}`), buff);
