...
servico 180 desbloqueio tim 356789123456789`
    ...
    const result = await run(
      `INSERT INTO servicos (cliente_jid, numero, nome, imei, descricao, valor, status)
       VALUES (?, ?, ?, ?, ?, ?, 'EM ANDAMENTO')`,
      [from, cliente.numero, cliente.nome, imei, descricao, valor]
    );
...
🏢 *CentralUnlocker*`
  ...
  if (cmd === "debito") {
    ...
    const cliente = await ensureCliente(from, nomeCliente);
    await run("UPDATE clientes SET saldo = saldo + ?, atualizado_em = CURRENT_TIMESTAMP WHERE jid = ?", [valor, from]);
    const atualizado = await get("SELECT * FROM clientes WHERE jid = ?", [from]);
...
🏢 *CentralUnlocker*`
  ...
  if (cmd === "pagou") {
    ...
    await run("UPDATE clientes SET saldo = ?, atualizado_em = CURRENT_TIMESTAMP WHERE jid = ?", [novoSaldo, from]);
    await run(
      "INSERT INTO pagamentos (cliente_jid, numero, nome, valor, origem) VALUES (?, ?, ?, ?, 'manual')",
      [from, cliente.numero, cliente.nome, valor]
    );
